#!/usr/bin/env bash
# Full SQLite backup next to the live DB (IMAGES_PATH/db-backups/).
#
# The gallery DB lives next to the images (see settings DATABASES), not in the
# git tree — archives go there too so a copy of IMAGES_PATH includes history.
#
# Policy:
...
#
# Cron (weekly, Sunday 03:45 — matesla uses 03:30):
#   45 3 * * 0 /home/mehdi/PycharmProjects/PicturesDjango/scripts/backup_db.sh >> /tmp/picturesdjango-db-backup.log 2>&1
#
set -euo pipefail

ROOT="$(cd "$(dirname "$0")/.." && pwd)"
ENV_FILE="${PICTURESDJANGO_ENV:-$ROOT/PicturesDjango/.env}"
PYTHON="${PICTURESDJANGO_PYTHON:-$ROOT/.venv/bin/python}"
KEEP="${KEEP:-4}"
MIN_AGE_DAYS="${MIN_AGE_DAYS:-6}"
FORCE="${FORCE:-0}"
ZSTD_LEVEL="${ZSTD_LEVEL:-3}"

# Resolve IMAGES_PATH: env override → .env → default
if [[ -z "${IMAGES_PATH:-}" && -f "$ENV_FILE" ]]; then
  IMAGES_PATH="$(
    grep -E '^[[:space:]]*IMAGES_PATH=' "$ENV_FILE" | head -1 \
      | sed -E 's/^[[:space:]]*IMAGES_PATH=//' \
      | sed -E 's/^["'\'']//; s/["'\'']$//'
  )"
fi
IMAGES_PATH="${PICTURESDJANGO_IMAGES_PATH:-${IMAGES_PATH:-/home/mehdi/Images}}"
DB_PATH="${PICTURESDJANGO_DB:-$IMAGES_PATH/db.sqlite3}"
BACKUP_DIR="${PICTURESDJANGO_BACKUP_DIR:-$IMAGES_PATH/db-backups}"
...
if [[ "$FORCE" != "1" ]]; then
  newest="$(
    find "$BACKUP_DIR" -maxdepth 1 -type f -name 'picturesdjango-db-*.sqlite3.zst' \
...
stamp="$(date +%Y-%m-%d)"
tmp_db="$(mktemp "$BACKUP_DIR/.picturesdjango-db-XXXXXX.sqlite3")"
out_zst="$BACKUP_DIR/picturesdjango-db-${stamp}.sqlite3.zst"
...
ls -lh "$BACKUP_DIR"/picturesdjango-db-*.sqlite3.zst 2>/dev/null || true
